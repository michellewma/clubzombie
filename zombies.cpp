...
private:
    ...
    int    m_row;
    int    m_col;
    bool   injured;     // boolean to see if the zombie has been clubbed
};
...
Zombie::Zombie(Arena* ap, int r, int c)
{
    ...
    m_row = r;
    m_col = c;
    injured = false; // initialize injured to false since zombies are untouched
}
...
bool Zombie::getAttacked(int dir)  // return true if dies
{
    ...
    // move zombie in dir and return false or return true if it can't
...
void Player::moveOrAttack(int dir)
{
    ...
    bool attackedZombie = false; // flag whether the player has encountered a zombie
    switch(dir)
    {
        case UP:
            // if there is a zombie above, attack it
    ...
    // if no attacks happened, move if possible
    if (!attackedZombie)
        m_arena->determineNewPosition(m_row, m_col, dir);
}
...
Arena::~Arena()
{
    ...
    // loop through the list of zombies and delete each one
...
int Arena::numZombiesAt(int r, int c) const
{
    ...
    // return the number of zombies at a given position
    int count = 0;
    for (int i = 0; i < m_nZombies; i++){
...
bool Arena::addZombie(int r, int c)
{
    ...
    Zombie* z = new Zombie(this, r, c);
    m_zombies[m_nZombies] = z;
    m_nZombies++;
    return true;
}
